...
public class Deneme extends Helper {
				...
				for (int j = i; j < path.size(); j++) {
					...
					if (p1_1.equals(p1)) {
						if (value.get(p2_2) == null) {
							...
							value.put(p2_2, "("+value.get(p2_2)+letter+")");
						}
					}
				}
				paths.put(p1, value);
	...
	public void operation() {
		fill_hashmap();
		System.out.println("S:"+S_start_state);
		System.out.println("A:"+A_final_state);
		String RE = "";
	...
	public String afs(HashMap<String, String> path, String RE) {
		for (String key : path.keySet()) {
			if (key.equals(A_final_state)) {
				...
				System.out.println(this.state+"-"+key+";"+key+"-"+this.state+"--->"+"("+path.get(key)+RE+")*");
